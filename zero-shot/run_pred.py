...
import numpy as np
import torch
from datasets import AST_Dataset, W2V2_Dataset, Whisper_Dataset, w2v2_collate_fn, AST_Test
...
if __name__ == "__main__":
    ...
    if 'w2v2' in config['training']['runname']:
        ...
        w_path = '/ds/audio/Speech_Quality_Datasets/weights/zeroshot/zs-w2v2_20260101_213833/zs-w2v2_20260101_213833mos_.tar'
        ...
        ds_tr = W2V2_Dataset(df_tr, config['training']['data_dir'], dim=config['dimension'].lower())
        ds_val = W2V2_Dataset(df_val, config['training']['data_dir'], dim=config['dimension'].lower())
        w_path = '/ds/audio/Speech_Quality_Datasets/weights/zeroshot/zs-HuBERT_20260101_210853/zs-HuBERT_20260101_210853mos_.tar'
        load_head_from_dp_checkpoint(w_path, model, device)
        ...
        ds_tr = W2V2_Dataset(df_tr, config['training']['data_dir'], dim=config['dimension'].lower())
        ds_val = W2V2_Dataset(df_val, config['training']['data_dir'], dim=config['dimension'].lower())
        w_path = '/ds/audio/Speech_Quality_Datasets/weights/zeroshot/zs-WavLM_20260101_210904/zs-WavLM_20260101_210904mos_.tar'
        load_head_from_dp_checkpoint(w_path, model, device)
        ...
        w_path = '/ds/audio/Speech_Quality_Datasets/weights/zeroshot/zs-Whisper_20260101_210917/zs-Whisper_20260101_210917mos_.tar'
        load_head_from_dp_checkpoint(w_path, model, device)

    else:
        model = AST()
        ds_val = AST_Test(df_val, config['training']['data_dir'], dim=config['dimension'].lower())
        #w_path = '/ds/audio/Speech_Quality_Datasets/weights/zeroshot/zs-AST_20260101_210917/zs-AST_20260101_210917mos_.tar'
        w_path = '/Users/wafaa/Code/psamd-master/zeroshot/scripts/zs-AST_20260101_210917mos_.tar'
        load_head_from_dp_checkpoint(w_path, model, device)
    ...
    collate_fn = get_collate_fn(config['training']['runname'], ds_val)
